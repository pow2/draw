...
public class SiteStylesDAO {
	
	private static Logger logger = LoggerFactory.getLogger(SiteStylesDAO.class);
	...
	private static List<String> getDBTags() {
		...
		try {
			...
			stmnt = con.prepareStatement("SELECT st.name "
					+ "FROM kerrigan.style st "
					+ "LIMIT ? OFFSET ?");
